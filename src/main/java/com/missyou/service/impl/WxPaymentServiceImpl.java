...
import com.github.wxpay.sdk.LWxPayConfig;
...
import com.missyou.util.HttpRequestProxy;
...
public class WxPaymentServiceImpl implements WxPaymentService {
    ...
    private Map<String, String> makePaySignature(Map<String, String> wxOrder) {
        ...
        wxPayMap.put("appId", WxPaymentServiceImpl.lWxPayConfig.getAppID());
        ...
        String sign;
        try {
            sign = WXPayUtil.generateSignature(wxPayMap, WxPaymentServiceImpl.lWxPayConfig.getKey(), WXPayConstants.SignType.HMACSHA256);
        } catch (Exception e) {
            e.printStackTrace();
            throw new ServerErrorException(9999);
        }
    ...
    public WXPay assembleWxPayConfig() {
        WXPay wxPay;
        try {
            wxPay = new WXPay(WxPaymentServiceImpl.lWxPayConfig);
        } catch (Exception ex) {
            throw new ServerErrorException(9999);
        }
        return wxPay;
